...
def generate_cyber_html(all_items, source_stats, date_str, time_str, now_str):
    ...
    # 生成来源标签 HTML
    source_tags_html = ""
    colors = ['#00d4ff', '#ff006e', '#8338ec', '#ffbe0b', '#06ffa5', '#ff5400', '#00f5d4', '#f72585']
    for idx, (source, count) in enumerate(sorted(source_stats.items(), key=lambda x: -x[1])):
        color = colors[idx % len(colors)]
        source_tags_html += f'''
            <div class="source-tag" style="--tag-color: {color}">
    ...
    web3_count = len([i for i in all_items if i.get("type") == "web3"])
    rss_count = len([i for i in all_items if i.get("type") == "rss"])
